...
class EditorCanvas(QGraphicsView):
    ...
    def mousePressEvent(self, event: QMouseEvent) -> None:
        ...
        if self._tool == Tool.TEXT:
            ...
            if accepted and text:
                ...
                item.setDefaultTextColor(self._style.stroke_color)
                font = item.font()
        ...
        if self._tool == Tool.CROP:
            ...
            if clicked_item is not None and clicked_item is self._crop_item:
                super().mousePressEvent(event)
                return
            self.cancel_crop()
            self._preview_item = self._create_preview_item(scene_pos)
    ...
    def _apply_crop(self, crop_rect: QRectF) -> None:
        ...
        crop_rect = crop_rect.intersected(self._scene.sceneRect()).normalized()
        if crop_rect.width() < 2 or crop_rect.height() < 2:
            return

        flattened = self.export_composited_pixmap()
        cropped = flattened.copy(crop_rect.toRect())
        self.cancel_crop()
    ...
    def paste_from_clipboard(self, view_pos: QPoint | None = None) -> None:
        ...
        if mime.hasText():
            ...
            if text:
                ...
                text_item.setDefaultTextColor(self._style.stroke_color)
                font = text_item.font()
                font.setPointSize(self._style.font_size)
                text_item.setFont(font)
